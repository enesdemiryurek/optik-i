...
function EditProductForm() {
    ...
    const fetchProduct = async () => {
      try {
        ...
        if (res.ok) {
          ...
          if (product.specs) {
            setSpecs({
              frameWidth: product.specs.frameWidth || '',
              lensWidth: product.specs.lensWidth || '',
              bridgeWidth: product.specs.bridgeWidth || '',
              templeLength: product.specs.templeLength || '',
              lensHeight: product.specs.lensHeight || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(`Ürün güncellenirken bir sorun oluştu. Lütfen bilgileri kontrol edip tekrar deneyin.\n\nSistem Notu: ${errorData?.error || res.statusText || 'Bilinmeyen Hata'}`);
      }
    } catch (err: any) {
  ...
  if (loading) {
    return <div className="min-h-screen flex items-center justify-center font-serif text-2xl bg-admin">Yükleniyor...</div>;
  }

  return (
    <div className="min-h-screen bg-admin">
      <nav className="glass sticky top-0 z-50 px-8 py-5 flex justify-between items-center mb-8 shadow-sm border-b border-border/50">
        <div className="font-serif text-xl tracking-widest uppercase flex items-center gap-3">
          <Link href="/admin/dashboard" className="w-8 h-8 rounded-full bg-white border border-border/50 flex items-center justify-center hover:bg-black hover:text-white transition-colors shadow-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="bg-white p-8 rounded-3xl border border-border/50 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input type="text" name="model" value={formData.model} onChange={handleChange} className="input-modern" required placeholder="Örn: RB3025" />
              </div>
            </div>

            <div className="mt-6">
              <label className="block text-xs font-bold label-caps mb-2 text-gray-700">Ürün Adı</label>
              <input type="text" name="name" value={formData.name} onChange={handleChange} className="input-modern" required placeholder="Örn: Aviator Classic" />
            </div>
          ...
          <div className="bg-white p-8 rounded-3xl border border-border/50 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="text" inputMode="decimal" pattern="[0-9.,]*" name="price" value={formData.price} onChange={handleChange} className="input-modern pl-8 font-mono" required placeholder="0.00" />
                </div>
              </div>
            </div>
            ...
            <div className="mt-6">
              ...
              <textarea name="description" value={formData.description} onChange={handleChange} className="input-modern" rows={4} placeholder="Ürün özelliklerini detaylı bir şekilde açıklayın..." />
            </div>
          </div>
            ...
            <div>
              <div className="border-2 border-dashed border-gray-300 p-8 text-center bg-gray-50 hover:bg-gray-100 transition-colors cursor-pointer relative rounded-2xl group overflow-hidden">
                ...
                <div className="flex flex-col items-center pointer-events-none">
                  <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center mb-4 group-hover:scale-110 transition-transform shadow-sm border border-border/50">
                  ...
                  <span className="font-bold label-caps text-sm text-gray-800">YENİ FOTOĞRAF SEÇ VEYA SÜRÜKLE</span>
                  <span className="text-xs text-muted mt-2 max-w-xs">Birden fazla fotoğraf seçebilirsiniz. İlk fotoğraf vitrin olacaktır.</span>
          ...
          <button type="submit" className="w-full btn-primary py-5 rounded-2xl text-lg uppercase tracking-wide">
...
export default function AdminProductEditPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-admin flex items-center justify-center font-serif text-2xl">Yükleniyor...</div>}>
      <EditProductForm />
    </Suspense>
  );
}
